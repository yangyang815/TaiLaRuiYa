...
for (const [id, sc] of Object.entries(SCENES)) {
  if (t.includes('scene: {"sky":"' )) { /* 已有则跳过单条判断在下方 */ }
  const anchor = '{ id: "' + id + '",'
  const check = 'scene: {"sky":"' + sc.sky
  if (t.includes(anchor + ' scene:') || t.includes('scene: {"sky":"' + sc.sky.slice(0, 20))) { ok++; continue }
  if (!t.includes(anchor)) { console.log('未找到', id); process.exit(1) }
...
console.log('scene 注入生效:', ok, '/ 8')
// 断言
if (!t.includes('scene: {"sky"')) { console.log('断言失败'); process.exit(1) }
